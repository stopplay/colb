 ...
 */
 ...
 jQuery(function( $ ) {
	...
	function touchStart(event,passedName) {
		...
		fingerCount = 1;//event.touches.length;
	...
	$("#superContainer").bind('touchend', function(event) {
		alert("touch end");
		//if(!event)
		//	return
		event.preventDefault();
	...
	$("#superContainer").bind('touchmove', function touchMove(event) {
		event.preventDefault();
		if ( event.touches.length == 1 ) {
			curX = event.touches[0].pageX;
			curY = event.touches[0].pageY;
	...
	$("#superContainer").bind('touchcancel', function(event) {
		//if(!event)
		//	return
		// reset the variables back to default values
		fingerCount = 0;
	...
	$("#superContainer").on('touchstart', function(event, passedName) {
		//alert("touch start");
		event.preventDefault();
		passedName = "superContainer";
		// disable the standard ability to select the touched object
	...
	$("#superContainer").on('touchend', function(event) {
		//alert("touch end");
		//if(!event)
		//	return
		event.preventDefault();
	...
	$("#superContainer").on('touchmove', function touchMove(event) {
		//event.preventDefault();
	...
	$("#superContainer").on('touchcancel', function(event) {
		//if(!event)
		//	return
		// reset the variables back to default values
		fingerCount = 0;
			...
			if(options.autoScrolling){
				...
				if (!isMoving) { //if theres any #
					...
					/*if(e.deltaX > 0){
						...
						if($('a.btn.next.link').length != 0){
							console.log("btn.NEXT.link .trigger");
							$('a.btn.next.link').trigger('click');
						}
						else{
							console.log("btn.NEXT.link .click");
							$('a.btn.next')[0].click();
						}
					}
					//scroll left
						...
						console.log("deltax < 0");
						if($('a.btn.prev.link').length != 0){
							console.log("btn.PREV.link .trigger");
							$('a.btn.prev.link').trigger('click');
						}
						else{
							console.log("btn.PREV.link .click");
							$('a.btn.prev')[0].click();
						}
					}*/
